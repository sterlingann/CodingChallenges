...
extension Get {
        ...
        var kURL = "http://api.giphy.com/v1/gifs/search?q=\(petChoice)&api_key=291f380b87884fc8996bd9d0078c42e3&limit=30&offset=\(callCount)"
        callCount += 30
        let newsURL = URL(string: kURL)
        let task = URLSession.shared.dataTask(with: newsURL!) {
            (data, response, error) in
            if error != nil {
                print("there is an error -SL GET1...", error ?? "")
            } else {
                do {
                ...
                } catch {print("caught", "kURL", kURL, "petChoice", petChoice, "newsURL", newsURL)}
            }
        }// URLSession...
        task.resume()
